...
class VerifyLogType(str, Enum): REQUEST = "request” SUCCESS = "success”
NO_GROUP = "no_group” INVALID_NICK = "invalid_nick”

class RankLogType(str, Enum): PROMOTE = "promote” DEMOTE = "demote”

class RankSummaryType(str, Enum): BULK_PROMOTE = "bulk_promote”
BULK_DEMOTE = "bulk_demote” # ———- 엠베드 ———- def make_verify_embed(
log_type: VerifyLogType, *, user: discord.abc.User | discord.Member |
None = None, roblox_nick: str | None = None, group_rank: str | None =
None, account_age_days: int | None = None, code: str | None = None,
new_nick: str | None = None, group_id: int | None = None, input_nick:
str | None = None, fail_reason: str | None = None, at_time: datetime |
None = None, ) -> discord.Embed: at_time = at_time or datetime.now()
...
def make_rank_log_embed( log_type: RankLogType, *, target_name: str,
old_rank: str, new_rank: str, executor: discord.abc.User |
discord.Member | None = None, ) -> discord.Embed: if log_type is
RankLogType.DEMOTE: title = "⬇️ 강등” desc = "멤버가 강등되었습니다.”
color = discord.Color.red() else: title = "⬆️ 승진” desc = "멤버가
승진되었습니다.” color = discord.Color.green()
...
def make_bulk_rank_summary_embed( summary_type: RankSummaryType, *,
role_name: str, total: int, success: int, failed: int, executor:
discord.abc.User | discord.Member | None = None, ) -> discord.Embed: if
summary_type is RankSummaryType.BULK_PROMOTE: title = "일괄 승진 완료”
color = discord.Color.green() desc = "여러 멤버 승진 작업이
완료되었습니다.” else: title = "일괄 강등 완료” color =
discord.Color.red() desc = "여러 멤버 강등 작업이 완료되었습니다.”
...
@bot.tree.command(name="인증”, description="로블록스 계정 인증을
시작합니다.”) @app_commands.describe(로블닉="로블록스 닉네임”) async def
verify(interaction: discord.Interaction, 로블닉: str): await
...
@app_commands.describe(역할="인증 역할”) async def
configure(interaction: discord.Interaction, 역할: discord.Role): if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
...
@bot.tree.command(name="역할목록”, description="서버 역할과 봇 역할을
10개씩 출력합니다.(관리자)”) async def role_all(interaction:
discord.Interaction):
...
@bot.tree.command(name="관리자지정”, description="관리자 역할 추가/제거
(개발자 전용)”) @app_commands.describe( 역할="추가할 관리자 역할”,
모드="add = 추가 / remove = 제거 / reset = 전체초기화” )
@app_commands.choices( 모드=[ app_commands.Choice(name="add”,
value="add”), app_commands.Choice(name="remove”, value="remove”),
app_commands.Choice(name="reset”, value="reset”), ] ) async def
set_admin_roles( interaction: discord.Interaction, 역할:
Optional[discord.Role], 모드: app_commands.Choice[str], ): if not
is_owner(interaction.user.id): await interaction.response.send_message(
"개발자만 사용할 수 있습니다.”, ephemeral=True ) return
...
@bot.tree.command(name="명단”, description="Roblox 그룹 역할 리스트를
보여줍니다.”) async def list_roles(interaction: discord.Interaction): if
not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="승진”, description="Roblox 그룹 랭크를 특정
역할로 변경합니다. (관리자)”) @app_commands.describe( username="Roblox
본닉”, role_name="그룹 역할 이름”, ) async def promote_cmd( interaction:
discord.Interaction, username: str, role_name: str, ): if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
    ...
    try:
        payload = {"username": username, "rank"
        data = resp.json()
...
@bot.tree.command(name="강등”, description="Roblox 그룹 랭크를 특정
역할로 변경합니다. (관리자)”) @app_commands.describe( username="Roblox
본닉”, role_name="그룹 역할 이름”, ) async def demote_to_role_cmd(
interaction: discord.Interaction, username: str, role_name: str, ): if
not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="일괄승진”, description="인증된 모든 유저를 특정
역할로 승진합니다. (관리자)”)
@app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe(role_name="변경할 그룹 역할 이름 또는 숫자”)
async def bulk_promote_to_role(interaction: discord.Interaction,
role_name: str): if not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="일괄강등”, description="인증된 모든 유저를 특정
역할로 변경합니다. (관리자)”)
@app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe(role_name="변경할 그룹 역할 이름 또는 숫자”)
async def bulk_demote_to_role(interaction: discord.Interaction,
role_name: str): if not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="강제인증해제”, description="유저의 인증을
해제합니다. (관리자)”) @app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe(user="인증 해제할 Discord 유저”) async def
unverify_user(interaction: discord.Interaction, user: discord.User): if
not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="동기화”, description="슬래시 명령어를 다시
동기화합니다. (관리자)”) async def sync_commands(interaction:
discord.Interaction): if not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용 가능합니다.”,
ephemeral=True) return
...
@bot.tree.command(name="강제인증”, description="유저를 강제로 인증
처리합니다. (관리자)”) @app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe( user="Discord 유저 멘션”, roblox_nick="Roblox
닉네임” ) async def force_verify(interaction: discord.Interaction, user:
discord.User, roblox_nick: str): if not is_admin(interaction.user):
...
@bot.tree.command(name="인증로그보기”, description="인증 기록을
확인합니다. (관리자)”) @app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe(최근="최근 N개 (기본 20)”) async def
view_verification_log(interaction: discord.Interaction, 최근: int = 20):
if not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command( name="일괄닉네임변경”, description="인증된 유저의
닉네임을 [랭크] 본닉 형식으로 변경합니다. (관리자)” )
@app_commands.guilds(discord.Object(id=GUILD_ID)) async def
bulk_nickname_change(interaction: discord.Interaction): if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="로그채널지정”, description="로그 채널을
설정합니다. (관리자)”) @app_commands.describe( 인증="인증 로그 채널”,
그룹변경="그룹변경 로그 채널”, 관리자="관리자 로그 채널”, 보안="보안
로그 채널”, 개발자="개발자 로그 채널”, ) async def set_log_channels(
interaction: discord.Interaction, 인증: discord.TextChannel | None =
None, 그룹변경: discord.TextChannel | None = None, 관리자:
discord.TextChannel | None = None, 보안: discord.TextChannel | None =
None, 개발자: discord.TextChannel | None = None, ): if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="블랙리스트”, description="블랙리스트 그룹을
관리합니다. (관리자)”) @app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe( group_id="Roblox 그룹 ID”, action="add (추가)
또는 remove (제거)”, ) async def manage_blacklist(interaction:
discord.Interaction, group_id: int, action: str = "add”): if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="블랙리스트목록”, description="블랙리스트 그룹
목록을 봅니다. (관리자)”)
@app_commands.guilds(discord.Object(id=GUILD_ID)) async def
view_blacklist(interaction: discord.Interaction): if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="역할전체변경”, description="모든 유저의 역할을
한 역할로 통일합니다. (위험)”) async def set_all_role(interaction:
discord.Interaction): guild = interaction.guild if guild.id != GUILD_ID:
await interaction.response.send_message("이 명령어는 지정된 서버에서만
사용할 수 있습니다.”, ephemeral=True) return
...
@bot.tree.command(name="장교역할”, description="장교 (영관급 ~ 장성급)
에게 부여할 역할을 설정합니다. (관리자)”)
@app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe(role="영관급 장교 역할”) async def
set_senior_officer_role(interaction: discord.Interaction, role:
discord.Role): if not is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@bot.tree.command(name="업데이트”, description="유저의 Discord 닉네임을
변경합니다. (관리자)”) @app_commands.guilds(discord.Object(id=GUILD_ID))
@app_commands.describe( user="디스코드 유저 멘션”, roblox_nick="로블록스
닉네임”, ) async def update_user( interaction: discord.Interaction,
user: discord.User, roblox_nick: str ): # 관리자 체크 if not
is_admin(interaction.user): await
interaction.response.send_message("관리자만 사용할 수 있습니다.”,
ephemeral=True) return
...
@tasks.loop(hours=6) async def sync_all_nicknames_task(): """6시간마다
전체 유저의 Roblox 정보를 동기화하고 닉네임 업데이트”"” try:
...
@sync_all_nicknames_task.before_loop async def
before_sync_all_nicknames_task(): await bot.wait_until_ready()

@tasks.loop(minutes=5) async def officer_role_sync_task(): """5분마다
인증된 유저의 랭크를 체크하여 위관급 장교 역할 자동 부여/해제”"” try:
guild = bot.get_guild(GUILD_ID) if not guild: return
...
@officer_role_sync_task.before_loop async def
before_officer_role_sync_task(): await bot.wait_until_ready()

@tasks.loop(seconds=5) async def rank_log_task(): """5분마다 그룹
가입자들의 랭크를 로그”"” try: cursor.execute("SELECT guild_id,
channel_id FROM rank_log_settings WHERE enabled=1”) settings =
cursor.fetchall()
...
@rank_log_task.before_loop async def before_rank_log_task(): await
bot.wait_until_ready()

TARGET_CUSTOM_EMOJI_IDS = [ 1471826252989071482, 1471875175791464643,
1471826069857243331, 1471826479754117264, 1471825935027408987 # 감지할
커스텀 이모지 ID]

ALLOWED_ROLE_IDS = [ 111111111111111111]

ALLOWED_USER_IDS = [ 794811652620156949, 1185946251171217519,
1468191965052141629, 1206574701380636692]

TIMEOUT_DURATION = timedelta(days=1) LOG_CHANNEL_ID =
1468191965052141629 # ==========================================

def contains_target_custom_emoji(message: discord.Message): if not
message.guild: return False
...
@bot.event async def on_message(message: discord.Message): if
